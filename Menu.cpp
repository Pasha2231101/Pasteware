	...
	namespace Functions
	{
		namespace Visuals
		{
			...
			void Menu::Initialize ( )
			{
				...
				Add ( "Основное", false, false );
				Add ( "Аимбот", false, false );
				Add ( "Триггербот", false, false );
				Add ( "Визуалы", false, false );
				Add ( "Разное", false, false );
				Add ( "Бинды", true, false );
				Add ( "Загрузить конфиг", false, true );
				Add ( "Сохранить конфиг", false, true );

				switch ( M_EntryPos )
				{
				case MENU_ENTRY::MAIN:
				{
					AddSubInt ( "Язык", &g_Vars.Main.Language, ENGLISH, RUSSIAN, 1, false, true,
				...
				case MENU_ENTRY::AIMBOT:
				{
					...
					case SUBMENU_ENTRY_AIMBOT::GLOBALS:
					{
						...
						AddSubSubBool ( "Сайлент", &g_Vars.Aim.Silent, false, false, true, false, false );
					...
					case SUBMENU_ENTRY_AIMBOT::RECOIL:
					{
						...
						AddSubSubFloat ( "Ость Y", &g_Vars.Recoil.Y, 0.0f, 3.0f, 0.01f, true, false, false, false, false );
				...
				case MENU_ENTRY::VISUALS:
				{
					...
					case SUBMENU_ENTRY_VISUALS::ESP:
					{
						...
						AddSubSubBool("СкринЕСП", &g_Vars.ESP.ScreenESP, false, false, true, false, false);
						...
						//AddSubSubBool("Рандомный цвет", &g_Vars.ESP.SoundRandomColor, false, false, true, false, false);
					...
					case SUBMENU_ENTRY_VISUALS::OTHER:
					{
						...
						AddSubSubBool("Флуд", &g_Vars.Other.SayFlood, false, false, true, false, false);
						AddSubSubInt("ВаллХак", &g_Vars.Other.WallHack, 0, 2, 1, false, false, false, false, false);
						AddSubSubBool("НочнойМод", &g_Vars.Other.NightMode, false, false, true, false, false);
						//AddSubSubBool("Rainbow", &g_Vars.Other.Rainbow1, false, true, true, false, false);
						AddSubSubBool("Неймстиллер(не работает)", &g_Vars.Other.NameStealer, false, true, true, false, false);
					...
					if (i == M_EntryPos)
					{
						if (!SubMenu)
						{
							Renderer::g_Drawing->FillArea(MenuBoxX, MenuBoxY, HeadBoxWidth, HeadBoxHeight,
								255, 0, 255, BoxSelectColor[3]);
						}

						Renderer::g_Drawing->FillArea(MenuBoxX, MenuBoxY, 2, HeadBoxHeight, 255, 0, 255, 200);
					}
						...
						if (i == S_EntryPos)
						{
							if (!SubSubMenu)
								Renderer::g_Drawing->FillArea(SubMenuBoxX, SubMenuBoxY, HeadBoxWidth,
									HeadBoxHeight, 255, 0, 255, BoxSelectColor[3]);
