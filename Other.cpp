...
namespace Client
{
	namespace Functions
	{
		namespace Visuals
		{
			...
			void Other::SayFlood()
			{
				static float LastSayTime;
				float TickCount = GetTickCount();

				if (((TickCount - LastSayTime) / 1000) > 0.75)
				{
					static int SayType = 0;

					switch (SayType)
					{
					case 0: SDK::Interfaces::g_pEngine->pfnCvar_Set("say", " currentmap"); SayType = 1; break;
					case 1: SDK::Interfaces::g_pEngine->pfnCvar_Set("say ", "nextmap"); 	SayType = 2; break;
					case 2: SDK::Interfaces::g_pEngine->pfnCvar_Set("say", "ff"); 		SayType = 0; break;
					}

					LastSayTime = TickCount;
				}
			}
			...
			void Other::DrawPunch ( )
			{
				...
				Renderer::g_Drawing->BoxCorner(x, y, 10, 10, 1, 255, 0, 0, 255);
			}
			...
			void Other::NameStealer()
			{
				static DWORD timer = GetTickCount();
					...
					for (int i = 1; i <= SDK::Interfaces::g_pEngine->GetMaxClients(); i++)
					{
						...
						if (g_Info->GetPlayerInfoByIndex(i).name == 0 || g_Info->GetPlayerInfoByIndex(i).name == "")
							continue;

						if (strlen(g_Info->GetPlayerInfoByIndex(i).name) <= 1)
							continue;

						strcpy_s(cNames[iCount++], g_Info->GetPlayerInfoByIndex(i).name);
					}
			...
			void Other::HUD_Redraw ( )
			{
				if ( g_Info->GetAliveLocal ( ) && !g_Info->GetBadWeapon ( ) )
				{
					...
					if ( g_Vars.Other.NameStealer ) NameStealer ( );
					if ( g_Vars.Other.SayFlood ) SayFlood ( );
				}
