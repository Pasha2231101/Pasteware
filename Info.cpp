...
namespace Client
{
	char *GetModelNameByIndex(int index)
	{
		DWORD HwBase = (DWORD)GetModuleHandle("hw.dll"); 			// for steam only
		DWORD getModelName = HwBase + 0x1201018 + (index - 1) * 0x250; 		// address cs steam

		if (getModelName != 0x0)
			return (char*)getModelName;
	}
	...
	Info::Info ( )
	{
		...
		for ( int PlayerID = 0; PlayerID <= 32; ++PlayerID )
		{
			...
			Player[PlayerID].Model = GetModelNameByIndex(PlayerID);
