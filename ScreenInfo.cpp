			...
			void ScreenInfo::DrawCopyright()
			{
				...
				if ( g_Vars.ScreenInfo.Copyright == 1.0f )
				{
					Renderer::g_Fonts->Print(SDK::Interfaces::g_Screen.iWidth - 118, 15, rainbow[0],
						rainbow[1], rainbow[2], 255, g_Vars.ScreenInfo.FontOutline ? FL_OUTLINE : FL_NONE, "Pasteware");
				}
				else if ( g_Vars.ScreenInfo.Copyright >= 2.0f )
				{
					Renderer::g_Fonts->Print ( SDK::Interfaces::g_Screen.iWidth / 2, 15, rainbow[0],
						rainbow[1], rainbow[2], 255, g_Vars.ScreenInfo.FontOutline ? FL_CENTER | FL_OUTLINE : FL_CENTER, "PasteWare" );
				}
			}
