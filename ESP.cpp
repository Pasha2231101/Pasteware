...
namespace Client
{
	namespace Functions
	{
		namespace Visuals
		{
			...
			BYTE ESP::RandomsColor[3];

			void ESP::RandomColor()
			{
			}
			...
			void ESP::PlayerWeapon ( cl_entity_s *Entity, const Vector2D &ScreenBot, const Vector2D &ScreenTop )
			{
				...
				if ( SequenceInfo[Entity->curstate.sequence] == 2 )
				{
					Renderer::g_Verdana->Print ( ( int )ScreenTop[0], ( int )y + 12, 255, 80, 80,
						255, g_Vars.ESP.FontOutline ? FL_CENTER | FL_OUTLINE : FL_CENTER, "Перезаряжается" );
				}
				else if ( SequenceInfo[Entity->curstate.sequence] == 5 )
				{
					Renderer::g_Verdana->Print ( ( int )ScreenTop[0], ( int )y + 12, 255, 80, 80,
						255, g_Vars.ESP.FontOutline ? FL_CENTER | FL_OUTLINE : FL_CENTER, "Ставит бомбу" );
				}
			...
			void ESP::ScreenESP(const int& Index)
			{
				byte r, g, b, a;
				bool RetFlag;
				...
				{
					Renderer::g_Drawing->DrawLine(vecTop[0], vecTop[1], vecBottom[0], vecBottom[1],
						BoxColor[0], BoxColor[1], BoxColor[2], 255);
				}
			}

			void ESP::PlayerSkeleton(const int &Index)
			{
			...
			void ESP::DrawPlayer ( cl_entity_s* Entity, int iIndex )
			{
				...
				if ( Utils::CalcScreen ( vOrigin, vScreenOrigin ) && Utils::CalcScreen ( vHead, vScreenHead ) )
				{
					...
					{
						Renderer::g_Drawing->DrawCoalBox(x, (int)ScreenTop[1], w, PlayerBoxHeight, BoxColor[0], BoxColor[1], BoxColor[2], 255);
						if (g_Vars.ESP.PlayerBoxOutline)
						{
							Renderer::g_Drawing->DrawCoalBoxOutLine(x, (int)ScreenTop[1], w, PlayerBoxHeight, BoxColor[0], BoxColor[1], BoxColor[2], 255);
						}
					}
				}
			}
				...
				for ( int Index = 0; Index < SoundIndex; ++Index )
				{
					...
					else
					{
						...
						if ( Utils::CalcScreen ( Sound[Index].vOrigin, SoundScreen ) && !Sound[Index].vOrigin.IsZero ( ) )
						{
							...
							else if(g_Vars.ESP.SoundRandomColor)
								Renderer::g_Drawing->Box(int(SoundScreen[0] - (size / 2.0f)), int(SoundScreen[1] - (size / 2.0f)),
								(int)size, (int)size, 1, RandomsColor[0], RandomsColor[1], RandomsColor[2], BYTE(Utils::Interp(float(Sound[Index].dwTime), float(GetTickCount64()), (float)SoundTime, 255.0f, 0.0f)));
			...
			void ESP::HUD_Redraw ( )
			{
				...
				if ( g_Vars.ESP.Player )
				{
					for ( int Index = 1; Index <= SDK::Interfaces::g_pEngine->GetMaxClients ( ); ++Index )
					{
						...
						if ( g_Info->GetValidationEntity ( Entity ) && Index != g_Info->GetLocalIndex ( ) )
						{
							...
							ScreenESP(Index);
							if (g_Vars.ESP.PlayerSkeleton)
								PlayerSkeleton(Index);
						}
					}
				}
				...
				if (g_Vars.ESP.SoundRandomColor)
					RandomColor();
			}
